<script>
	...
	resumeInfo.subscribe((value) => {
		info = value;
		items = info.sections.projects.items;
	});
...
<div id="projects-section-container">
	{#each items as project}
		<div class="project-container" style="margin-bottom: {textLineMargin * 5}px;">
			<p
				class="project-name"
				style="{textSettingsStyle} font-size: {fontSize * 1.2}rem; margin-bottom: {textLineMargin *
					2}px"
			>
				{project.name}
			</p>
			<p class="project-description" style="{textSettingsStyle} font-size: {fontSize}rem;">
				{project.description}
			</p>
			<a
				class="project-link"
				href={project.link}
				style="{textSettingsStyle} font-size: {fontSize}rem;"
			>
				{project.link}
			</a>
		</div>
